...
namespace Moonfish.Graphics.Input
{
    ...
    public class ConstrainedTranslationGizmo : TranslationGizmo
    {
        ...
        public Matrix4 PivotWorldMatrix
        {
            set
            {
                _pivot = value.ExtractTranslation();
            }
        }
        ...
        public override Matrix4 WorldMatrix
        {
            get { return CalculateWorldMatrix(position, rotation, scale)*Matrix4.CreateFromQuaternion(_transform); }
            set { base.WorldMatrix = value; }
        }
        ...
        protected override void Commit()
        {
            rotation = (CalculateWorldMatrix(position, rotation, scale) * Matrix4.CreateFromQuaternion(_transform)).ExtractRotation();
            position = (CalculateWorldMatrix(position, rotation, scale) * Matrix4.CreateFromQuaternion(_transform)).ExtractTranslation();
            rotation.Normalize();
            _transform = Quaternion.Identity;
            base.Commit();
        }
        ...
        protected override void Transform(SceneMouseMoveEventArgs args)
        {
            ...
            var transform = Quaternion.FromAxisAngle(normal, (float)translationProjectionLength).Normalized();// (positionVector, translationVector);
