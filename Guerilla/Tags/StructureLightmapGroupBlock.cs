    ...
    public class StructureLightmapGroupBlockBase  : IGuerilla
    {
        ...
        public int Write(System.IO.BinaryWriter binaryWriter, Int32 nextAddress)
        {
            using(binaryWriter.BaseStream.Pin())
            {
                ...
                Guerilla.WriteBlockArray<StructureLightmapPaletteColorBlock>(binaryWriter, sectionPalette, nextAddress);
                Guerilla.WriteBlockArray<StructureLightmapPaletteColorBlock>(binaryWriter, writablePalettes, nextAddress);
                binaryWriter.Write(bitmapGroup);
                Guerilla.WriteBlockArray<LightmapGeometrySectionBlock>(binaryWriter, clusters, nextAddress);
                Guerilla.WriteBlockArray<LightmapGeometryRenderInfoBlock>(binaryWriter, clusterRenderInfo, nextAddress);
                Guerilla.WriteBlockArray<LightmapGeometrySectionBlock>(binaryWriter, poopDefinitions, nextAddress);
                Guerilla.WriteBlockArray<StructureLightmapLightingEnvironmentBlock>(binaryWriter, lightingEnvironments, nextAddress);
                Guerilla.WriteBlockArray<LightmapVertexBufferBucketBlock>(binaryWriter, geometryBuckets, nextAddress);
                Guerilla.WriteBlockArray<LightmapGeometryRenderInfoBlock>(binaryWriter, instanceRenderInfo, nextAddress);
                Guerilla.WriteBlockArray<LightmapInstanceBucketReferenceBlock>(binaryWriter, instanceBucketRefs, nextAddress);
                Guerilla.WriteBlockArray<LightmapSceneryObjectInfoBlock>(binaryWriter, sceneryObjectInfo, nextAddress);
                Guerilla.WriteBlockArray<LightmapInstanceBucketReferenceBlock>(binaryWriter, sceneryObjectBucketRefs, nextAddress);
