	...
	class GuerillaBlockClass : GuerillaBlockClassBase
	{
			...
			: this(tag.Definition.Name.ToPascalCase().ToAlphaNumericToken())
		{
			...
			if (hasParent)
			{
				...
				UsedNames = new TokenDictionary(parentClass.UsedNames);
					...
					case MoonfishFieldType.FieldArrayStart:
						{
							...
							var member =
								new CodeMemberField(
									...
									UsedNames.GenerateValidToken(GenerateFieldName(field)));
							GenerateSummary(member);
							member.Attributes = MemberAttributes.Public;
		...
		private void GenerateEnumField(MoonfishTagField field)
		{
			...
			var name = TargetClass.Name.Replace("Block", "") + enumBlockClass.GetFieldName();
			var fieldMember = new CodeMemberField(
				new CodeTypeReference(
					enumBlockClass.TargetClass.Name),
					GenerateName(MemberAttributes.Public, null, false, 
				                 field.Strings.Name, name)
				)
			{
				Attributes = MemberAttributes.Public
			};
		...
		{
			var contains = TargetClass.Members.OfType<CodeTypeDeclaration>()
