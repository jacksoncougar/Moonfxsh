...
namespace Moonfish.Guerilla
{
    ...
    public class Guerilla
    {
        ...
        /// <summary>The number of tag layouts in the tag layout table.</summary>
        private const int NumberOfTagLayouts = 120;

        /// <summary>
        ///     Name of the h2 language library used for localizing user interface
        ///     strings.
        /// </summary>
        ...
        private static readonly StringBuilder _str = new StringBuilder(0x1000);
        ...
        public static int GetFieldSize(field_type type)
        {
            switch (type)
            {
                ...
                case field_type._field_moonfish_ident:
                    return Marshal.SizeOf(typeof (TagIdent));
        ...
        /// <summary>
        /// Calls the matching Preprocess Funciton for the given name on a 
        /// collection of fields and returns the resulting field collection.
        /// </summary>
        /// <param name="name">The name of the block the fields belong to.</param>
        ...
        {
            if (string.Empty == ToTypeName(value))
                return string.Empty;
        ...
        protected static void InitializeNamespaceDictionary()
        {
            ...
            Namespaces =
                new List<string>(new[]
                {globalNamespace, globalGeometryNamespace, structureNamespace, structureBspNamespace});
        ...
        private static void LoadPostProcessFunctions()
        {
            ...
            {
                object[] attributes = method.GetCustomAttributes(typeof (GuerillaPreProcessFieldsMethodAttribute), false);
                ...
                foreach (GuerillaPreProcessFieldsMethodAttribute attribute in attributes)
                {
                    try
                    {
                        preProcessFieldsFunctions[attribute.BlockName] =
                            (ProcessFieldsDelegate) Delegate.CreateDelegate(typeof (ProcessFieldsDelegate), method);
                    }
                    catch (Exception)
                    {
                        System.Diagnostics.Debug.WriteLine(
                            $"Failed to create pre-preprocess delegate from {method.Name} for {attribute.BlockName}");
                    }
                }
            }
        }
